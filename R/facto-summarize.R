...
#'  dudi, pca, coa, acm, mfa [ade4]; ca [ca package].
...
{
    # check element
    if (!element %in% c("row", "col", "var",
        "ind", "quali.var", "quanti.var",
        "group", "partial.axes"))
        stop("The argument element should be one of \"row\", \"col\", \"var\", \"ind\", \"quali.var\", \"quanti.var\", \"group\", \"partial.axes\".")
    ...
    if (facto_class == "CA") {
        if (element %in% c("ind", "row"))
            elmt <- get_ca_row(X) else if (element %in% c("var", "col"))
            elmt <- get_ca_col(X)
    } else if (facto_class == "PCA") {
        if (element %in% c("var", "col"))
            elmt <- get_pca_var(X) else if (element %in% c("ind", "row"))
            elmt <- get_pca_ind(X)
    } else if (facto_class == "MCA") {
        if (element %in% c("var", "col"))
            elmt <- get_mca_var(X) else if (element %in% c("ind", "row"))
            elmt <- get_mca_ind(X)
    } else if (facto_class == "MFA") {
        if (element %in% c("quanti.var",
            "col"))
            elmt <- get_mfa_quanti_var(X)
        if (element %in% c("quali.var",
            "col"))
            elmt <- get_mfa_quali_var(X)
        if (element %in% c("group", "col"))
            elmt <- get_mfa_group(X)
        if (element %in% c("partial.axes",
            "col"))
            elmt <- get_mfa_partial_axes(X) else if (element %in% c("ind", "row"))
            elmt <- get_mfa_ind(X)
    }
    ...
    if (max(axes) > ncol(elmt$coord))
        stop("The value of the argument axes is incorrect. ",
            "The number of axes in the data is: ",
    ...
    # 1.Extract the coordinates x, y and
    # coord
    if ("coord" %in% result) {
        dd <- data.frame(elmt$coord[, axes,
            drop = FALSE])
    ...
    if ("contrib" %in% result) {
        ...
        if (length(axes) > 1) {
            eig <- get_eigenvalue(X)[axes,
                1]
            # Adjust variable contributions by the
            # Dimension eigenvalues
            contrib <- t(apply(contrib,
                1, function(var.contrib,
                  pc.eig) {
                  var.contrib * pc.eig
                }, eig))
            contrib <- apply(contrib, 1,
                sum)
        }
    ...
    res <- cbind.data.frame(name = name,
        res)
    if (!is.null(select))
