#' Print summaory of sMFA results.
#'
#' @param x
#' @param file
#' @param sep
#' @param ...
#'
#' @return List of sMFA results.
#' @export
#'
#' @examples
print.sMFA <- function (x, file = NULL, sep = ";", ...){
  res.smfa <- x
  if (!inherits(res.smfa, "sMFA")) stop("non convenient data")
  cat("**Results of the Specific Multiple Factor Analysis (sMFA)**\n")
  cat("The analysis was performed on", nrow(res.smfa$call$X),
      "individuals, described by", ncol(res.smfa$call$X), "variables\n")
  ...
  indice <- 7
  if (!is.null(res.smfa["ind.sup"]$ind.sup)){
  ...
  if (!is.null(res.smfa["quanti.var.sup"]$quanti.var.sup)){
  ...
  print(res[1:indice,])
  if (!is.null(file)) {
    write.infile(res.smfa,file = file, sep=sep)
