                  ...
                  kk=Inf, pattern = "Fehlender Wert|kann ich nicht sagen",...)
{
  ...
  if (inherits(res, "PCA") | inherits(res, "MCA") | inherits(res,"MFA") | inherits(res, "HMFA") | inherits(res, "FAMD") | inherits(res.sauv, "sMFA") | inherits(res.sauv, "sHMFA")) {
  ...
  if ((kk!=Inf)&(graph)) {
    ...
    if (inherits(res.sauv, "sMFA")) plot(res.sauv, col.ind=as.numeric(data.clust$clust),invisible=c("quali","quali.sup"),new.plot=TRUE,cex=0.8)
