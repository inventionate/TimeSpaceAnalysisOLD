...
print.spHMFA <- function (x, file = NULL, sep = ";", ...){
  ...
  cat("There are", nrow(res.sphmfa$ind$coord), "individuals\n\n")
  ...
  indice <- 4
  if (!is.null(res.sphmfa["quanti.var"]$quanti.var)){
    indice <- indice + 1
    res[indice, ] <- c("$quanti.var", "results for the quantitative variables")
  }
  if (!is.null(res.sphmfa["quali.var"]$quali.var)){
    indice <- indice + 1
    res[indice, ] <- c("$quali.var", "results for the categorical variables")
  }
  print(res[1:indice,])
  if (!is.null(file)) {
    write.infile(res.sphmfa,file = file, sep=sep)
