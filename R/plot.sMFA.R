#' Plot specific Multiple Factor Analysis results.
#'
#' @param x
#' @param axes
#' @param choix
...
#' @param ...
#'
#' @return Returns the standard plots of sMFA results.
#' @export
#'
#' @examples
plot.sMFA=function (x, axes = c(1, 2), choix = c("ind","var","group","axes","freq"), ellipse = NULL, ellipse.par = NULL,
...
{
  res.smfa <- x
  if (!inherits(res.smfa, "sMFA")) stop("non convenient data")
  ...
  lab.x <- paste("Dim ",axes[1]," (",format(res.smfa$eig[axes[1],2],nsmall=2,digits=2),"%)",sep="")
  lab.y <- paste("Dim ",axes[2]," (",format(res.smfa$eig[axes[2],2],nsmall=2,digits=2),"%)",sep="")
  group <- res.smfa$call$group
  nbre.grpe <- length(group)
  type <- res.smfa$call$type
  num.group.sup = NULL
  if (!is.null(res.smfa$call$num.group.sup)) {
    num.group.sup <- res.smfa$call$num.group.sup
  ...
  if (choix == "axes") {
    ...
    abline(v = 0, lty = 2,...)
    abline(h = 0, lty = 2,...)
    coord.axes <- res.smfa$partial.axes$coord[, axes, drop = FALSE]
    if (!is.null(select)) {
      if (mode(select)=="numeric") selection <- select
      else {
        if (sum(rownames(res.smfa$partial.axes$coord)%in%select)!=0) selection <- which(rownames(res.smfa$partial.axes$coord)%in%select)
        else {
          if (grepl("contrib",select)) selection <- (rev(order(res.smfa$partial.axes$contrib[,axes[1],drop=FALSE]*res.smfa$eig[axes[1],1]+res.smfa$partial.axes$contrib[,axes[2],drop=FALSE]*res.smfa$eig[axes[2],1])))[1:min(nrow(res.smfa$partial.axes$coord),sum(as.integer(unlist(strsplit(select,"contrib"))),na.rm=T))]
          # 		    if (grepl("contrib",select)) selection <- (rev(order(apply(res.smfa$partial.axes$contrib[,axes],1,sum))))[1:min(nrow(res.smfa$partial.axes$coord),sum(as.integer(unlist(strsplit(select,"contrib"))),na.rm=T))]
          if (grepl("coord",select)) selection <- (rev(order(apply(res.smfa$partial.axes$coord[,axes]^2,1,sum))))[1:min(nrow(res.smfa$partial.axes$coord),sum(as.integer(unlist(strsplit(select,"coord"))),na.rm=T))]
    ...
    if (habillage == "group") {
      if (is.null(col.hab) | length(col.hab) < length(group)) {
        if (is.null(res.smfa$call$num.group.sup)) col.hab <- 2:(length(group) + 1)
        else {
          col.hab[which(!(1:length(group))%in%(res.smfa$call$num.group.sup))] <- 2:(1+length(group)-length(res.smfa$call$num.group.sup))
          col.hab[res.smfa$call$num.group.sup] <- length(group)-length(res.smfa$call$num.group.sup)+1+(1:length(res.smfa$call$num.group.sup))
        }
      }
      i = 1
      couleur.axes <- col.hab[i]
      auxil = strsplit(rownames(res.smfa$partial.axes$coord)[1], ".", fixed = TRUE)[[1]]
      auxil2 = auxil[length(auxil)]
      for (j in 2:nrow(res.smfa$partial.axes$coord)) {
        auxil = strsplit(rownames(res.smfa$partial.axes$coord)[j], ".", fixed = TRUE)[[1]]
      ...
      couleur.axes <- NULL
      for (i in 1:length(group)) couleur.axes <- c(couleur.axes, rep("black", ncol(res.smfa$partial.axes$coord)))
    }
    posi <- coll <- NULL
    if (!is.null(select)){
    ...
    if (habillage == "group") legend("topleft", legend = rownames(res.smfa$group$Lg)[-length(rownames(res.smfa$group$Lg))], text.col = unique(couleur.axes), ...)
  }

  if (choix == "group") {
    coord.actif <- res.smfa$group$coord[, axes, drop = FALSE]
    if (!is.null(res.smfa$group$coord.sup))  coord.illu <- res.smfa$group$coord.sup[, axes, drop = FALSE]
    ...
    if (!is.null(select)) {
      if (mode(select)=="numeric") selection <- select
        ...
        if (sum(rownames(res.smfa$group$coord)%in%select)+sum(rownames(res.smfa$group$coord.sup)%in%select)!=0) selection <- which(rownames(res.smfa$group$coord)%in%select)
        else {
          if (grepl("contrib",select)) selection <- (rev(order(res.smfa$group$contrib[,axes[1]]*res.smfa$eig[axes[1],1]+res.smfa$group$contrib[,axes[2]]*res.smfa$eig[axes[2],1])))[1:min(nrow(res.smfa$group$coord),sum(as.integer(unlist(strsplit(select,"contrib"))),na.rm=T))]
          if (grepl("coord",select)) selection <- (rev(order(apply(res.smfa$group$coord[,axes]^2,1,sum))))[1:min(nrow(res.smfa$group$coord),sum(as.integer(unlist(strsplit(select,"coord"))),na.rm=T))]
          if (grepl("cos2",select)) {
            if (sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T)>=1) selection <- (rev(order(apply(res.smfa$group$cos2[,axes],1,sum))))[1:min(nrow(res.smfa$group$coord),sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T))]
            else selection <- which(apply(res.smfa$group$cos2[,axes],1,sum)>sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T))
          }
          if (is.integer(select)) selection <- select
        }
      }
    }
    if ((!is.null(select))&(!is.null(res.smfa$group$coord.sup))) {
      if (mode(select)=="numeric") selectionS <- select
      else {
        if (sum(rownames(res.smfa$group$coord)%in%select)+sum(rownames(res.smfa$group$coord.sup)%in%select)!=0) selectionS <- which(rownames(res.smfa$group$coord.sup)%in%select)
        else {
          if (grepl("contrib",select)) selectionS <- NULL
          if (grepl("coord",select)) selectionS <- (rev(order(apply(res.smfa$group$coord.sup[,axes]^2,1,sum))))[1:min(nrow(res.smfa$group$coord.sup),sum(as.integer(unlist(strsplit(select,"coord"))),na.rm=T))]
          if (grepl("cos2",select)) {
            if (sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T)>=1) selectionS <- (rev(order(apply(res.smfa$group$cos2.sup[,axes],1,sum))))[1:min(nrow(res.smfa$group$coord.sup),sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T))]
            else selectionS <- which(apply(res.smfa$group$cos2.sup[,axes],1,sum)>sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T))
          }
    ...
    if (is.null(col.hab)) {
      ...
      if (!is.null(res.smfa$group$coord.sup))  col.hab = c(col.hab, rep("darkolivegreen", nrow(coord.illu)))
    }
    ...
    if (!is.null(res.smfa$group$coord.sup)) {
  ...
  if (choix == "var") {
    ...
    if (habillage == "group") {
      if (is.null(col.hab) | length(col.hab) < length(group[type == "c"])){
        if (!is.null(res.smfa$call$num.group.sup)){
          col.hab[which(!(1:length(group))%in%(res.smfa$call$num.group.sup))] <- 2:(1+length(group)-length(res.smfa$call$num.group.sup))
          col.hab[res.smfa$call$num.group.sup] <- length(group)-length(res.smfa$call$num.group.sup)+1+(1:length(res.smfa$call$num.group.sup))
          col <- c(1+rep(which(res.smfa$call$nature.group[-res.smfa$call$num.group.sup]=="quanti"),times=group[which(res.smfa$call$nature.group=="quanti")]),length(group)-length(res.smfa$call$num.group.sup)+1+rep(which((res.smfa$call$nature.group[res.smfa$call$num.group.sup])=="quanti.sup"),times=group[which(res.smfa$call$nature.group=="quanti.sup")]))
        } else {
    ...
    abline(v = 0, lty = 2, ...)
    abline(h = 0, lty = 2, ...)
    if ((!is.null(select))&(!is.null(res.smfa["quanti.var"]$quanti.var))) {
      if (mode(select)=="numeric") selection <- select
      else {
        if (sum(rownames(res.smfa$quanti.var$coord)%in%select)+sum(rownames(res.smfa$quanti.var.sup$coord)%in%select)!=0) selection <- which(rownames(res.smfa$quanti.var$coord)%in%select)
        else {
          if (grepl("contrib",select)) selection <- (rev(order(res.smfa$quanti.var$contrib[,axes[1],drop=FALSE]*res.smfa$eig[axes[1],1]+res.smfa$quanti.var$contrib[,axes[2],drop=FALSE]*res.smfa$eig[axes[2],1])))[1:min(nrow(res.smfa$quanti.var$coord),sum(as.integer(unlist(strsplit(select,"contrib"))),na.rm=T))]
          # 		      if (grepl("contrib",select)) selection <- (rev(order(apply(res.smfa$quanti.var$contrib[,axes],1,sum))))[1:min(nrow(res.smfa$quanti.var$coord),sum(as.integer(unlist(strsplit(select,"contrib"))),na.rm=T))]
          if (grepl("coord",select)) selection <- (rev(order(apply(res.smfa$quanti.var$coord[,axes]^2,1,sum))))[1:min(nrow(res.smfa$quanti.var$coord),sum(as.integer(unlist(strsplit(select,"coord"))),na.rm=T))]
          if (grepl("cos2",select)) {
            if (sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T)>=1) selection <- (rev(order(apply(res.smfa$quanti.var$cos2[,axes],1,sum))))[1:min(nrow(res.smfa$quanti.var$coord),sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T))]
            else selection <- which(apply(res.smfa$quanti.var$cos2[,axes],1,sum)>sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T))
          }
          if (is.integer(select)) selection <- select
        }
      }
    }
    if ((!is.null(select))&(!is.null(res.smfa$quanti.var.sup))) {
      if (mode(select)=="numeric") selectionS <- select
      else {
        if (sum(rownames(res.smfa$quanti.var$coord)%in%select)+sum(rownames(res.smfa$quanti.var.sup$coord)%in%select)!=0) selectionS <- which(rownames(res.smfa$quanti.var.sup$coord)%in%select)
        else {
          if (grepl("contrib",select)) selectionS <- NULL
          if (grepl("coord",select)) selectionS <- (rev(order(apply(res.smfa$quanti.var.sup$coord[,axes]^2,1,sum))))[1:min(nrow(res.smfa$quanti.var.sup$coord),sum(as.integer(unlist(strsplit(select,"coord"))),na.rm=T))]
          if (grepl("cos2",select)) {
            if (sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T)>=1) selectionS <- (rev(order(apply(res.smfa$quanti.var.sup$cos2[,axes],1,sum))))[1:min(nrow(res.smfa$quanti.var.sup$coord),sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T))]
            else selectionS <- which(apply(res.smfa$quanti.var.sup$cos2[,axes],1,sum)>sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T))
          }
          if (is.integer(select)) selectionS <- select
        }
      }
    ...
    if (!is.null(res.smfa["quanti.var"]$quanti.var)){
      coll <- col[1:nrow(res.smfa["quanti.var"]$quanti.var$coord)]
      if (lab.var) labe <- rownames(res.smfa["quanti.var"]$quanti.var$coord)
      else  labe <- rep("",nrow(res.smfa["quanti.var"]$quanti.var$coord))
    }
    if (!is.null(res.smfa$quanti.var.sup)&is.na(test.invisible[2])){
      if (lab.var) labe2 <- rownames(res.smfa$quanti.var.sup$coord)
      else  labe2 <- rep("",nrow(res.smfa$quanti.var.sup$coord))
    ...
    if (habillage == "group" & is.na(test.invisible[1]) & is.na(test.invisible[2]))
      legend("topleft", legend = rownames(res.smfa$group$Lg[-nrow(res.smfa$group$Lg),,drop=FALSE])[type == "c"], text.col = col.hab[type == "c"], cex = 0.8*par("cex"))
    if (habillage == "group" & is.na(test.invisible[1]) & !is.na(test.invisible[2])){
      if ("quanti.sup"%in%res.smfa$call$nature.var) legend("topleft", legend = rownames(res.smfa$group$Lg[-c(num.group.sup, nrow(res.smfa$group$Lg)),,drop=FALSE])[type.act == "c"],
                                                          text.col = col.hab[which(!((1:length(group))%in%res.smfa$call$num.group.sup))[type.act == "c"]], cex = 0.8*par("cex"))
      else legend("topleft", legend = rownames(res.smfa$group$Lg[-nrow(res.smfa$group$Lg), ])[type == "c"],
                  text.col = col.hab[type == "c"], cex = 0.8*par("cex"))
    }
    if (habillage == "group" & !is.na(test.invisible[1]) & is.na(test.invisible[2])){
      if ("quanti"%in%res.smfa$call$nature.var) legend("topleft", legend = rownames(res.smfa$group$Lg[num.group.sup,,drop=FALSE])[type.sup == "c"], text.col = col.hab[res.smfa$call$num.group.sup[type.sup == "c"]], cex = 0.8*par("cex"))
      else legend("topleft", legend = rownames(res.smfa$group$Lg[num.group.sup,,drop=FALSE])[type.sup == "c"], text.col = col.hab[res.smfa$call$num.group.sup[type.sup == "c"]], cex = 0.8*par("cex"))
    }
    nrow.coord.var <- 0
    coo <- posi <- NULL
    if ((!is.null(res.smfa["quanti.var"]$quanti.var))&(is.na(test.invisible[1]))){
      col[1:nrow(res.smfa$quanti.var$cor[,axes,drop=FALSE])] <- "transparent"
    }
    if (!is.null(res.smfa["quanti.var"]$quanti.var)){
      ### modif 2016-02-16
      coord.var <- res.smfa$quanti.var$cor[,axes,drop=FALSE]
      coo <- coord.var
      if (length(which(apply(res.smfa$quanti.var$cos2[, axes,drop=FALSE],1,sum, na.rm = TRUE) < lim.cos2.var))>0) col[which(apply(res.smfa$quanti.var$cos2[, axes,drop=FALSE],1,sum, na.rm = TRUE) < lim.cos2.var)] <- "transparent"
    ...
    if ((!is.null(res.smfa$quanti.var.sup$coord))& (is.na(test.invisible[2]))){
      col[nrow(coo)+(1:nrow(res.smfa$quanti.var.sup$cor[,axes,drop=FALSE]))] <- "transparent"
    }

      if (!is.null(res.smfa$quanti.var.sup$coord)){
        coord.quanti <- res.smfa$quanti.var.sup$cor[ ,axes,drop=FALSE]
        coo <- rbind(coo,coord.quanti)
        if (length(which(apply(res.smfa$quanti.var.sup$cos2[, axes,drop=FALSE],1,sum, na.rm = TRUE) < lim.cos2.var))>0) col[nrow(coo)-nrow(coord.quanti)+which(apply(res.smfa$quanti.var.sup$cos2[, axes,drop=FALSE],1,sum, na.rm = TRUE) < lim.cos2.var)]<-"transparent"
  ...
  if (choix=="freq"){
    ...
    coord.col <- res.smfa$freq$coord[, axes, drop = FALSE]
    coord.row <- res.smfa$ind$coord[, axes]
    coord.row.sup <- coord.col.sup <- NULL
    if (!is.null(res.smfa$ind.sup)) coord.row.sup <- res.smfa$ind.sup$coord[, axes, drop = FALSE]
    if (!is.null(res.smfa$freq.sup)) coord.col.sup <- res.smfa$freq.sup$coord[, axes, drop = FALSE]
    ...
    if (!is.null(select)) {
      if (mode(select)=="numeric") selection <- select
        ...
        if (sum(rownames(res.smfa$freq.sup$coord)%in%select)+sum(rownames(res.smfa$freq$coord)%in%select)+sum(rownames(res.smfa$ind$coord)%in%select)+sum(rownames(res.smfa$ind.sup$coord)%in%select)!=0) selection <- which(rownames(res.smfa$ind$coord)%in%select)
        else {
          if (grepl("contrib",select)) selection <- (rev(order(res.smfa$ind$contrib[,axes[1],drop=FALSE]*res.smfa$eig[axes[1],1]+res.smfa$ind$contrib[,axes[2],drop=FALSE]*res.smfa$eig[axes[2],1])))[1:min(nrow(res.smfa$ind$coord),sum(as.integer(unlist(strsplit(select,"contrib"))),na.rm=T))]
          # 		    if (grepl("contrib",select)) selection <- (rev(order(apply(res.smfa$ind$contrib[,axes],1,sum))))[1:min(nrow(res.smfa$ind$coord),sum(as.integer(unlist(strsplit(select,"contrib"))),na.rm=T))]
          if (grepl("inertia",select)) selection <- (rev(order(apply(res.smfa$ind$within.inertia[,axes],1,sum))))[1:min(nrow(res.smfa$ind$coord),sum(as.integer(unlist(strsplit(select,"inertia"))),na.rm=T))]
          if (grepl("coord",select)) selection <- (rev(order(apply(res.smfa$ind$coord[,axes]^2,1,sum))))[1:min(nrow(res.smfa$ind$coord),sum(as.integer(unlist(strsplit(select,"coord"))),na.rm=T))]
          if (grepl("cos2",select)) {
            if (sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T)>=1) selection <- (rev(order(apply(res.smfa$ind$cos2[,axes],1,sum))))[1:min(nrow(res.smfa$ind$coord),sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T))]
            else selection <- which(apply(res.smfa$ind$cos2[,axes],1,sum)>sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T))
          }
          if (is.integer(select)) selection <- select
        }
      }
    }
    if ((!is.null(select))&(!is.null(res.smfa$ind.sup$coord))) {
      if (mode(select)=="numeric") selectionS <- select
      else {
        if (sum(rownames(res.smfa$freq.sup$coord)%in%select)+sum(rownames(res.smfa$freq$coord)%in%select)+sum(rownames(res.smfa$ind$coord)%in%select)+sum(rownames(res.smfa$ind.sup$coord)%in%select)!=0) selectionS <- which(rownames(res.smfa$ind.sup$coord)%in%select)
        else {
          if (grepl("contrib",select)) selectionS <- NULL
          if (grepl("inertia",select)) selectionS <- (rev(order(apply(res.smfa$ind.sup$within.inertia[,axes]^2,1,sum))))[1:min(nrow(res.smfa$ind.sup$coord),sum(as.integer(unlist(strsplit(select,"inertia"))),na.rm=T))]
          if (grepl("coord",select)) selectionS <- (rev(order(apply(res.smfa$ind.sup$coord[,axes]^2,1,sum))))[1:min(nrow(res.smfa$ind.sup$coord),sum(as.integer(unlist(strsplit(select,"coord"))),na.rm=T))]
          if (grepl("cos2",select)) {
            if (sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T)>=1) selectionS <- (rev(order(apply(res.smfa$ind.sup$cos2[,axes],1,sum))))[1:min(nrow(res.smfa$ind.sup$coord),sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T))]
            else selectionS <- which(apply(res.smfa$ind.sup$cos2[,axes],1,sum)>sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T))
          }
          if (is.integer(select)) selectionS <- select
        }
      }
    }
    if ((!is.null(select))&(!is.null(res.smfa$freq$coord))) {
      if (mode(select)=="numeric") selectionC <- select
      else {
        if (sum(rownames(res.smfa$freq.sup$coord)%in%select)+sum(rownames(res.smfa$freq$coord)%in%select)+sum(rownames(res.smfa$ind$coord)%in%select)+sum(rownames(res.smfa$ind.sup$coord)%in%select)!=0) selectionC <- which(rownames(res.smfa$freq$coord)%in%select)
        else {
          if (grepl("contrib",select)) selectionC <- (rev(order(res.smfa$freq$contrib[,axes[1],drop=FALSE]*res.smfa$eig[axes[1],1]+res.smfa$freq$contrib[,axes[2],drop=FALSE]*res.smfa$eig[axes[2],1])))[1:min(nrow(res.smfa$freq$coord),sum(as.integer(unlist(strsplit(select,"contrib"))),na.rm=T))]
          if (grepl("coord",select)) selectionC <- (rev(order(apply(res.smfa$freq$coord[,axes]^2,1,sum))))[1:min(nrow(res.smfa$freq$coord),sum(as.integer(unlist(strsplit(select,"coord"))),na.rm=T))]
          if (grepl("cos2",select)) {
            if (sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T)>=1) selectionC <- (rev(order(apply(res.smfa$freq$cos2[,axes],1,sum))))[1:min(nrow(res.smfa$freq$coord),sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T))]
            else selectionC <- which(apply(res.smfa$freq$cos2[,axes],1,sum)>sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T))
          }
          if (is.integer(select)) selectionC <- select
        }
      }
    }
    if ((!is.null(select))&(!is.null(res.smfa$freq.sup$coord))) {
      if (mode(select)=="numeric") selectionCS <- select
      else {
        if (sum(rownames(res.smfa$freq.sup$coord)%in%select)+sum(rownames(res.smfa$freq$coord)%in%select)+sum(rownames(res.smfa$ind$coord)%in%select)+sum(rownames(res.smfa$ind.sup$coord)%in%select)!=0) selectionCS <- which(rownames(res.smfa$freq.sup$coord)%in%select)
        else {
          if (grepl("contrib",select)) selectionCS <- NULL
          if (grepl("coord",select)) selectionCS <- (rev(order(apply(res.smfa$freq.sup$coord[,axes]^2,1,sum))))[1:min(nrow(res.smfa$freq.sup$coord),sum(as.integer(unlist(strsplit(select,"coord"))),na.rm=T))]
          if (grepl("cos2",select)) {
            if (sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T)>=1) selectionCS <- (rev(order(apply(res.smfa$freq.sup$cos2[,axes],1,sum))))[1:min(nrow(res.smfa$freq.sup$coord),sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T))]
            else selectionCS <- which(apply(res.smfa$freq.sup$cos2[,axes],1,sum)>sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T))
          }
    ...
    if (!is.null(res.smfa$freq.sup) & is.na(test.invisible[4])) {
    ...
    if (!is.null(res.smfa$ind.sup) & is.na(test.invisible[3])) {
    ...
    if (!shadowtext) points(coo[, 1], y = coo[, 2], pch = ipch, col = coll, ...)
    if (habillage == "group") legend("topleft", legend = rownames(res.smfa$group$Lg[-nrow(res.smfa$group$Lg), ])[type == "f"], text.col = col.hab, cex = 0.8*par("cex"))
  }
  ...
  if (choix == "ind") {
    ...
    nb.ind.actif <- nrow(res.smfa$ind$coord)
    nb.ind.illu <- 0
    if (!is.null(res.smfa$ind.sup)) nb.ind.illu <- nrow(res.smfa$ind.sup$coord)
    nb.ind <- nb.ind.actif + nb.ind.illu
    coord.ind <- res.smfa$ind$coord[, axes, drop = FALSE]
    coord.ind.partiel <- res.smfa$ind$coord.partiel[, axes, drop = FALSE]
    coord.ind.sup <- NULL
    if (!is.null(res.smfa$ind.sup)) {
      coord.ind.sup <- res.smfa$ind.sup$coord[, axes, drop = FALSE]
      coord.ind.partiel.sup <- res.smfa$ind.sup$coord.partiel[, axes, drop = FALSE]
    }
    coord.quali <- coord.quali.sup <- coord.quali.partiel <- coord.quali.sup.partiel <- NULL
    nrow.coord.quali <- 0
    if (!is.null(res.smfa["quali.var"]$quali.var)) {
      coord.quali <- res.smfa$quali.var$coord[, axes, drop = FALSE]
      coord.quali.partiel <- res.smfa$quali.var$coord.partiel[, axes, drop = FALSE]
      nrow.coord.quali <- nrow(coord.quali)
    }
    if (!is.null(res.smfa["quali.var.sup"])) {
      coord.quali.sup <- res.smfa$quali.var.sup$coord[, axes, drop = FALSE]
      coord.quali.partiel.sup <- res.smfa$quali.var.sup$coord.partiel[, axes, drop = FALSE]
    }
    ...
    if (!is.null(partial)) {
      if (length(partial) == 1) {
        if (partial == "all") {
          ...
          if (!is.null(res.smfa["quali.var.sup"]$quali.var.sup))
    ...
    if (is.null(xlim)) {
      ...
      if (!is.null(res.smfa$ind.sup) & is.na(test.invisible[2])) xmin <- min(xmin, coord.ind.sup[, 1])
      if (!is.null(res.smfa$ind.sup) & is.na(test.invisible[2])) xmax <- max(xmax, coord.ind.sup[, 1])
      ...
      if (is.na(test.invisible[1])) xmax <- max(xmax, coord.ind.partiel[unlist(lapply(group.ind.actif,
                                                                                      function(k) seq(nbre.grpe * (k - 1) + 1, length = nbre.grpe))), 1])
      if (!is.null(res.smfa$ind.sup) & is.na(test.invisible[2])) xmin <- min(xmin, coord.ind.partiel.sup[unlist(lapply(group.ind.sup,
                                                                                                                      function(k) seq(nbre.grpe * (k - 1) + 1, length = nbre.grpe))), 1])
      if (!is.null(res.smfa$ind.sup) & is.na(test.invisible[2]))
        xmax <- max(xmax, coord.ind.partiel.sup[unlist(lapply(group.ind.sup, function(k) seq(nbre.grpe * (k - 1) + 1, length = nbre.grpe))),
                                                1])
      if (!is.null(res.smfa["quali.var"]$quali.var) & is.na(test.invisible[3]))
      ...
      if (!is.null(res.smfa["quali.var"]$quali.var) & is.na(test.invisible[3]))
        xmax <- max(xmax, coord.quali.partiel[unlist(lapply(group.quali,
                                                            function(k) seq(nbre.grpe * (k - 1) + 1, length = nbre.grpe))),
                                              1])
      if (!is.null(res.smfa$quali.var.sup) & is.na(test.invisible[3]))
    ...
    if (!is.null(select)) {
      if (mode(select)=="numeric") selection <- select
        ...
        if (sum(rownames(res.smfa$ind$coord)%in%select)!=0) selection <- which(rownames(res.smfa$ind$coord)%in%select)
        else {
          if (grepl("contrib",select)) selection <- (rev(order(res.smfa$ind$contrib[,axes[1],drop=FALSE]*res.smfa$eig[axes[1],1]+res.smfa$ind$contrib[,axes[2],drop=FALSE]*res.smfa$eig[axes[2],1])))[1:min(nrow(res.smfa$ind$coord),sum(as.integer(unlist(strsplit(select,"contrib"))),na.rm=T))]
          # 		    if (grepl("contrib",select)) selection <- (rev(order(apply(res.smfa$ind$contrib[,axes],1,sum))))[1:min(nrow(res.smfa$ind$coord),sum(as.integer(unlist(strsplit(select,"contrib"))),na.rm=T))]
          if (grepl("dist",select)) selection <- (rev(order(res.smfa$ind$dist)))[1:min(nrow(res.smfa$ind$coord),sum(as.integer(unlist(strsplit(select,"dist"))),na.rm=T))]
          if (grepl("coord",select)) selection <- (rev(order(apply(res.smfa$ind$coord[,axes]^2,1,sum))))[1:min(nrow(res.smfa$ind$coord),sum(as.integer(unlist(strsplit(select,"coord"))),na.rm=T))]
          if (grepl("cos2",select)) {
            if (sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T)>=1) selection <- (rev(order(apply(res.smfa$ind$cos2[,axes],1,sum))))[1:min(nrow(res.smfa$ind$coord),sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T))]
            else selection <- which(apply(res.smfa$ind$cos2[,axes],1,sum)>sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T))
    ...
    if (habillage == "group") {
      ...
      if (!is.null(res.smfa$ind.sup)) col.ind.sup <- c(rep(1, nb.ind - nb.ind.actif), rep(col.hab, nb.ind - nb.ind.actif))
      if (length(group[type == "n"]) != 0) col.quali <- c(rep(1, sum(res.smfa$call$group.mod[type == "n"])), rep(col.hab, sum(res.smfa$call$group.mod[type == "n"])))
      if (!is.null(res.smfa$quali.var.sup)) col.quali.sup <- c(rep(1, sum(res.smfa$call$group.mod[num.group.sup][type.sup == "n"])), rep(col.hab, sum(res.smfa$call$group.mod[num.group.sup][type.sup == "n"])))
    ...
    if (habillage == "ind") {
      ...
      if (!is.null(res.smfa$ind.sup)) col.ind.sup <- c(col.hab[(nb.ind.actif + 1):nb.ind], rep(col.hab[(nb.ind.actif + 1):nb.ind], each = nbre.grpe))
      if (length(group[type == "n"]) != 0) col.quali <- col.quali.sup <- rep("black", (1 + nbre.grpe) * sum(res.smfa$call$group.mod[type == "n"]))
    ...
    if ((habillage != "none") & (habillage != "ind") & (habillage != "group")) {
      ...
      for (i in group.act) {
        if (type[i] == "n") {
          for (k in 1:ncol(res.smfa$separate.analyses[[i]]$call$X)) nbre.modalite <- c(nbre.modalite, nlevels(res.smfa$separate.analyses[[i]]$call$X[, k]))
          if (i == 1) liste.quali <- c(liste.quali, colnames(res.smfa$call$X[1:group[1]]))
          else liste.quali <- c(liste.quali, colnames(res.smfa$call$X[(sum(group[1:(i - 1)]) + 1):sum(group[1:i])]))
        }
      }
      if (!is.null(num.group.sup)) {
        for (i in num.group.sup) {
          if (type[i] == "n") {
            if (i == 1) liste.quali.sup <- c(liste.quali.sup, colnames(res.smfa$call$X[1:group[1]]))
            else liste.quali.sup <- c(liste.quali.sup, colnames(res.smfa$call$X[(sum(group[1:(i - 1)]) + 1):sum(group[1:i])]))
            for (k in 1:ncol(res.smfa$separate.analyses[[i]]$call$X)) nbre.modalite.sup <- c(nbre.modalite.sup, nlevels(res.smfa$separate.analyses[[i]]$call$X[, k]))
          }
        }
      }
      if (is.double(habillage)) nom.quali <- colnames(res.smfa$call$X)[habillage]
      else nom.quali = habillage
      if (!(nom.quali %in% c(liste.quali,liste.quali.sup))) stop("The variable ", habillage, " is not qualitative")
      modalite <- levels(as.factor(res.smfa$call$X[, nom.quali]))
      col.ind <- as.numeric(as.factor(res.smfa$call$X[, nom.quali]))
      if (is.null(col.hab) | length(col.hab) != length(modalite))  col.hab <- 2:(1 + length(modalite))
      col.ind <- col.hab[col.ind]
      if (!is.null(res.smfa$call$ind.sup)) {
        col.ind.sup <- col.ind[res.smfa$call$ind.sup]
        col.ind <- col.ind[-res.smfa$call$ind.sup]
      ...
      col.quali <- rep("black", sum(res.smfa$call$group.mod[type == "n"]))
      ...
      col.quali.sup <- rep("black", sum(res.smfa$call$group.mod[(type == "n")%in%num.group.sup]))
    ...
    if ((!is.null(partial)) & (habillage == "group"))
      legend("topleft", legend = rownames(res.smfa$group$Lg)[-c(num.group.sup,
                                                               length(rownames(res.smfa$group$Lg)))], lty = 1:length(rownames(res.smfa$group$Lg)[-c(num.group.sup,
                                                                                                                                                  length(rownames(res.smfa$group$Lg)))]), text.col = col.hab,
             col = col.hab, cex = par("cex")*0.8)
    if ((!is.null(partial)) & (habillage != "group"))
      legend("topleft", legend = rownames(res.smfa$group$Lg)[-c(num.group.sup,
                                                               length(rownames(res.smfa$group$Lg)))], lty = 1:length(rownames(res.smfa$group$Lg)[-c(num.group.sup,
                                                                                                                                                  length(rownames(res.smfa$group$Lg)))]), cex = par("cex")*0.8)
    if ((habillage != "none") & (habillage != "ind") & (habillage !=
                                                        "group"))
      legend("topleft", legend = levels(res.smfa$call$X[,
