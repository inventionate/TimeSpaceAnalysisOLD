...
.scale_ca <- function(data, res.ca, element="row", type="symmetric", axes = 1:2)
{
  ...
  if(!is.null(data)){
    ...
    else if(element %in% c("col", "quanti.var")) res <- .scale_ca_col(data, res.ca, type, axes)
    else if(element %in% c("col", "quali.var")) res <- .scale_ca_col(data, res.ca, type, axes)
    else if(element %in% c("col", "group")) res <- .scale_ca_col(data, res.ca, type, axes)
    else if(element %in% c("col", "partial.axes")) res <- .scale_ca_col(data, res.ca, type, axes)
...
.get_ca_mass <- function(res.ca, element){
  ...
  # FactoMiner
  else if(inherits(res.ca, c("CA", "MCA", "MFA"))){
