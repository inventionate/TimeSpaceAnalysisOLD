...
#' @param ...
#'
#' @return Returns a list of sMCA results.
#' @export
#'
#' @examples
print.sMCA <- function (x, file = NULL, sep = ";", ...){
  res.smca <- x
  if (!inherits(res.smca, "sMCA")) stop("non convenient data")
  cat("**Results of the Multiple Correspondence Analysis (MCA)**\n")
  cat("The analysis was performed on", nrow(res.smca$call$X),
      "individuals, described by", ncol(res.smca$call$X), "variables\n")
  ...
  print(res[1:indice,])
  if (!is.null(file)) {
    write.infile(res.smca,file = file, sep=sep)
