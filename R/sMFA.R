...
#' @param tab.comp
#'
#' @return Returns a list of sMFA results.
#' @export
#'
#' @examples
sMFA <- function (base, group, type = rep("s",length(group)), excl = NULL, ind.sup = NULL, ncp = 5, name.group = NULL, num.group.sup = NULL, graph = TRUE, weight.col.mfa = NULL, row.w = NULL, axes=c(1,2),tab.comp=NULL){
  ...
  for (g in 1:nbre.group) {
    ...
    if (type[g] == "n") {
      ...
      if(is.null(excl[[g]])) res.separe[[g]] <- MCA(aux.base, ind.sup = ind.sup, ncp=ncp, graph = FALSE, row.w=row.w)
      # @info Anpassungen für sMCA
      else res.separe[[g]] <- sMCA(aux.base, excl = excl[[g]], ind.sup = ind.sup, ncp=ncp, graph = FALSE, row.w=row.w)
    }
  ...
  for (g in 1:nbre.group) {
    ...
    if (type[g] == "n") {
      ...
      # @info sMCA Adaption
      # Gewicht der illustrierenden Kategorien entfernen
      # print(excl[[g]])
      # Für die Berechnung der contribution NUll definieren.
      if(!is.null(excl[[g]])) poids.tmp[excl[[g]]] <- 0
      # ponderation.zero <- ponderation
      # ponderation.zero[(ind.grpe.mod + 1):(ind.grpe.mod + group.mod[g])] <- poids.tmp/(res.separe[[g]]$eig[1,1] * group[g])
      # Für die Berechnung des Modells einen sehr kleinen Wert definieren.
    ...
    for (i in 1:nbre.group) {
      if (i%in%num.group.sup){
        ...
        if (is.null(data.group.sup)) {
          ...
          sup.inidces <- which(indices == group[num.group.sup])
          sup.names <- colnames(base)[sup.inidces]
          sup.mod.indices <- (1+nb.of.var):(nb.of.var+group.mod[i])
          structuring.factors <- list(sup.inidces, sup.names, sup.mod.indices)
        }
  ...
  # @info sMCA Adaption (Gewichte nach der Berechnung auf 0 setzen aka sMCA)
  res.globale <- sPCA(data.pca, scale.unit = FALSE, col.w = ponderation, row.w=row.w,ncp = ncp.tmp, ind.sup = ind.sup, quali.sup = aux.quali.sup.indice, structuring.factors = structuring.factors, quanti.sup = data.group.sup.indice, graph = FALSE)
    ...
    if ((type[g] == "c")||(type[g]=="f")) {
      ...
      for (v in 1:nb.var) {
        ...
        if(is.null(excl[[g]])) {
          ...
          # Ein großes Problem, da Kategorien hier entfernt werden müssen, die nicht immer eine weniger sind.
          # Anzahl der Modalitäten ohne Selektion
          nb.mod.orig <- c(nb.mod.orig, nlevels(res.separe[[g]]$call$X[, v]))
          # Die untere Grenze festlegen
          nb.mod.low <- nb.mod.high
          # Die obere Grenze festlegen
          nb.mod.high <- nb.mod.high + nlevels(res.separe[[g]]$call$X[, v])
          # Selektor für die auszuschließenden Modalitäten definieren
  ...
  if (!is.null(ind.quali)) {
    ...
    for (g in 1:nbre.group) {
      if (type[g] =="n"){
        ...
        # @info sMCA Adaption
        # Passive Modalitäten als Gesamtliste erzeugen.
        if(!is.null(excl[[g]])) {
    ...
    if (!is.null(ind.col.sup)) {
      ...
      # @info Anspassungen für die Ergebnisse der Strukturierten Datenanalyse
      variance.quali.sup <-  res.globale$quali.sup$variance[, 1:ncp,drop=FALSE]
      within.variance.quali.sup <- res.globale$quali.sup$within[, 1:ncp,drop=FALSE]
      between.variance.quali.sup <- res.globale$quali.sup$between[, 1:ncp,drop=FALSE]
      total.variance.quali.sup <- res.globale$quali.sup$total[, 1:ncp,drop=FALSE]
      eta2.sup <- res.globale$quali.sup$eta2[, 1:ncp,drop=FALSE]
      # Anspassungen Ende
    ...
    if (!is.null(ind.col.act)) {
      # coord.quali.act <- coord.quali[1:8,,drop=FALSE]
      # print(nrow(coord.quali))
      # print(coord.quali[2,])
      coord.quali.act <- coord.quali[ind.col.act,,drop=FALSE]
      # print(coord.quali.act[2,])
  ...
  cum.mrate <- rep(100, nrow(resultats$eig))
