                   ...
                   unselect = 0.7,shadowtext=FALSE,...) 
{
  ...
  lab.x <- paste("Dim ",axes[1]," (",format(res.spmfa$eig[axes[1],2],nsmall=2,digits=2),"%)",sep="")
  lab.y <- paste("Dim ",axes[2]," (",format(res.spmfa$eig[axes[2],2],nsmall=2,digits=2),"%)",sep="")
  group <- res.spmfa$call$group
  nbre.grpe <- length(group)
  type <- res.spmfa$call$type
  num.group.sup = NULL
  if (!is.null(res.spmfa$call$num.group.sup)) {
    num.group.sup <- res.spmfa$call$num.group.sup
  ...
  if (choix == "axes") {
    ...
    abline(v = 0, lty = 2,...)
    abline(h = 0, lty = 2,...)
    coord.axes <- res.spmfa$partial.axes$coord[, axes, drop = FALSE]
    if (!is.null(select)) {
      if (mode(select)=="numeric") selection <- select
      else {
        if (sum(rownames(res.spmfa$partial.axes$coord)%in%select)!=0) selection <- which(rownames(res.spmfa$partial.axes$coord)%in%select)
        else {
          if (grepl("contrib",select)) selection <- (rev(order(res.spmfa$partial.axes$contrib[,axes[1],drop=FALSE]*res.spmfa$eig[axes[1],1]+res.spmfa$partial.axes$contrib[,axes[2],drop=FALSE]*res.spmfa$eig[axes[2],1])))[1:min(nrow(res.spmfa$partial.axes$coord),sum(as.integer(unlist(strsplit(select,"contrib"))),na.rm=T))]
    ...
    if (habillage == "group") {
      if (is.null(col.hab) | length(col.hab) < length(group)) {
        if (is.null(res.spmfa$call$num.group.sup)) col.hab <- 2:(length(group) + 1)
        else {
          col.hab[which(!(1:length(group))%in%(res.spmfa$call$num.group.sup))] <- 2:(1+length(group)-length(res.spmfa$call$num.group.sup))
          col.hab[res.spmfa$call$num.group.sup] <- length(group)-length(res.spmfa$call$num.group.sup)+1+(1:length(res.spmfa$call$num.group.sup))
        }
      }
      i = 1
      couleur.axes <- col.hab[i]
      auxil = strsplit(rownames(res.spmfa$partial.axes$coord)[1], ".", fixed = TRUE)[[1]]
      auxil2 = auxil[length(auxil)]
      for (j in 2:nrow(res.spmfa$partial.axes$coord)) {
        auxil = strsplit(rownames(res.spmfa$partial.axes$coord)[j], ".", fixed = TRUE)[[1]]
      ...
      couleur.axes <- NULL
      for (i in 1:length(group)) couleur.axes <- c(couleur.axes, rep("black", ncol(res.spmfa$partial.axes$coord)))
    }
    posi <- coll <- NULL
    if (!is.null(select)){
  ...
  if (choix == "group") {
    ...
    if (!is.null(select)) {
      if (mode(select)=="numeric") selection <- select
        ...
        if (sum(rownames(res.spmfa$group$coord)%in%select)+sum(rownames(res.spmfa$group$coord.sup)%in%select)!=0) selection <- which(rownames(res.spmfa$group$coord)%in%select)
        else {
          if (grepl("contrib",select)) selection <- (rev(order(res.spmfa$group$contrib[,axes[1]]*res.spmfa$eig[axes[1],1]+res.spmfa$group$contrib[,axes[2]]*res.spmfa$eig[axes[2],1])))[1:min(nrow(res.spmfa$group$coord),sum(as.integer(unlist(strsplit(select,"contrib"))),na.rm=T))]
          if (grepl("coord",select)) selection <- (rev(order(apply(res.spmfa$group$coord[,axes]^2,1,sum))))[1:min(nrow(res.spmfa$group$coord),sum(as.integer(unlist(strsplit(select,"coord"))),na.rm=T))]
          if (grepl("cos2",select)) {
            if (sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T)>=1) selection <- (rev(order(apply(res.spmfa$group$cos2[,axes],1,sum))))[1:min(nrow(res.spmfa$group$coord),sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T))]
    ...
    if ((!is.null(select))&(!is.null(res.spmfa$group$coord.sup))) {
      if (mode(select)=="numeric") selectionS <- select
        ...
        if (sum(rownames(res.spmfa$group$coord)%in%select)+sum(rownames(res.spmfa$group$coord.sup)%in%select)!=0) selectionS <- which(rownames(res.spmfa$group$coord.sup)%in%select)
        else {
          if (grepl("contrib",select)) selectionS <- NULL
          if (grepl("coord",select)) selectionS <- (rev(order(apply(res.spmfa$group$coord.sup[,axes]^2,1,sum))))[1:min(nrow(res.spmfa$group$coord.sup),sum(as.integer(unlist(strsplit(select,"coord"))),na.rm=T))]
          if (grepl("cos2",select)) {
            if (sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T)>=1) selectionS <- (rev(order(apply(res.spmfa$group$cos2.sup[,axes],1,sum))))[1:min(nrow(res.spmfa$group$coord.sup),sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T))]
  ...
  if (choix == "var") {
    ...
    if (habillage == "group") {
      if (is.null(col.hab) | length(col.hab) < length(group[type == "c"])){
        if (!is.null(res.spmfa$call$num.group.sup)){
          col.hab[which(!(1:length(group))%in%(res.spmfa$call$num.group.sup))] <- 2:(1+length(group)-length(res.spmfa$call$num.group.sup))
          col.hab[res.spmfa$call$num.group.sup] <- length(group)-length(res.spmfa$call$num.group.sup)+1+(1:length(res.spmfa$call$num.group.sup))
          col <- c(1+rep(which(res.spmfa$call$nature.group[-res.spmfa$call$num.group.sup]=="quanti"),times=group[which(res.spmfa$call$nature.group=="quanti")]),length(group)-length(res.spmfa$call$num.group.sup)+1+rep(which((res.spmfa$call$nature.group[res.spmfa$call$num.group.sup])=="quanti.sup"),times=group[which(res.spmfa$call$nature.group=="quanti.sup")]))
        } else {
    ...
    abline(v = 0, lty = 2, ...)
    abline(h = 0, lty = 2, ...)
    if ((!is.null(select))&(!is.null(res.spmfa["quanti.var"]$quanti.var))) {
      if (mode(select)=="numeric") selection <- select
      else {
        if (sum(rownames(res.spmfa$quanti.var$coord)%in%select)+sum(rownames(res.spmfa$quanti.var.sup$coord)%in%select)!=0) selection <- which(rownames(res.spmfa$quanti.var$coord)%in%select)
        else {
          if (grepl("contrib",select)) selection <- (rev(order(res.spmfa$quanti.var$contrib[,axes[1],drop=FALSE]*res.spmfa$eig[axes[1],1]+res.spmfa$quanti.var$contrib[,axes[2],drop=FALSE]*res.spmfa$eig[axes[2],1])))[1:min(nrow(res.spmfa$quanti.var$coord),sum(as.integer(unlist(strsplit(select,"contrib"))),na.rm=T))]
          # 		      if (grepl("contrib",select)) selection <- (rev(order(apply(res.spmfa$quanti.var$contrib[,axes],1,sum))))[1:min(nrow(res.spmfa$quanti.var$coord),sum(as.integer(unlist(strsplit(select,"contrib"))),na.rm=T))]
          if (grepl("coord",select)) selection <- (rev(order(apply(res.spmfa$quanti.var$coord[,axes]^2,1,sum))))[1:min(nrow(res.spmfa$quanti.var$coord),sum(as.integer(unlist(strsplit(select,"coord"))),na.rm=T))]
          if (grepl("cos2",select)) {
            if (sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T)>=1) selection <- (rev(order(apply(res.spmfa$quanti.var$cos2[,axes],1,sum))))[1:min(nrow(res.spmfa$quanti.var$coord),sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T))]
    ...
    if ((!is.null(select))&(!is.null(res.spmfa$quanti.var.sup))) {
      if (mode(select)=="numeric") selectionS <- select
        ...
        if (sum(rownames(res.spmfa$quanti.var$coord)%in%select)+sum(rownames(res.spmfa$quanti.var.sup$coord)%in%select)!=0) selectionS <- which(rownames(res.spmfa$quanti.var.sup$coord)%in%select)
        else {
          if (grepl("contrib",select)) selectionS <- NULL
          if (grepl("coord",select)) selectionS <- (rev(order(apply(res.spmfa$quanti.var.sup$coord[,axes]^2,1,sum))))[1:min(nrow(res.spmfa$quanti.var.sup$coord),sum(as.integer(unlist(strsplit(select,"coord"))),na.rm=T))]
          if (grepl("cos2",select)) {
            if (sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T)>=1) selectionS <- (rev(order(apply(res.spmfa$quanti.var.sup$cos2[,axes],1,sum))))[1:min(nrow(res.spmfa$quanti.var.sup$coord),sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T))]
    ...
    if (!is.null(res.spmfa["quanti.var"]$quanti.var)&is.na(test.invisible[1])){
      ...
      if (lab.var) labe <- rownames(res.spmfa["quanti.var"]$quanti.var$coord)
      else  labe <- rep("",nrow(res.spmfa["quanti.var"]$quanti.var$coord))
    }
    if (!is.null(res.spmfa$quanti.var.sup)&is.na(test.invisible[2])){
      if (lab.var) labe2 <- rownames(res.spmfa$quanti.var.sup$coord)
    ...
    if (habillage == "group" & !is.na(test.invisible[1]) & is.na(test.invisible[2])){
      if ("quanti"%in%res.spmfa$call$nature.var) legend("topleft", legend = rownames(res.spmfa$group$Lg[num.group.sup, ])[type.sup == "c"], text.col = col.hab[res.spmfa$call$num.group.sup[type.sup == "c"]], cex = 0.8*par("cex"))
      else legend("topleft", legend = rownames(res.spmfa$group$Lg[num.group.sup, ])[type.sup == "c"], text.col = col.hab[res.spmfa$call$num.group.sup[type.sup == "c"]], cex = 0.8*par("cex"))
    }
    nrow.coord.var <- 0
    coo <- posi <- NULL
  ...
  if (choix=="freq"){
    ...
    coord.col <- res.spmfa$freq$coord[, axes, drop = FALSE]
    coord.row <- res.spmfa$ind$coord[, axes]
    ...
    if (!is.null(select)) {
      if (mode(select)=="numeric") selection <- select
        ...
        if (sum(rownames(res.spmfa$freq.sup$coord)%in%select)+sum(rownames(res.spmfa$freq$coord)%in%select)+sum(rownames(res.spmfa$ind$coord)%in%select)+sum(rownames(res.spmfa$ind.sup$coord)%in%select)!=0) selection <- which(rownames(res.spmfa$ind$coord)%in%select)
        else {
          if (grepl("contrib",select)) selection <- (rev(order(res.spmfa$ind$contrib[,axes[1],drop=FALSE]*res.spmfa$eig[axes[1],1]+res.spmfa$ind$contrib[,axes[2],drop=FALSE]*res.spmfa$eig[axes[2],1])))[1:min(nrow(res.spmfa$ind$coord),sum(as.integer(unlist(strsplit(select,"contrib"))),na.rm=T))]
          ...
          if (grepl("inertia",select)) selection <- (rev(order(apply(res.spmfa$ind$within.inertia[,axes],1,sum))))[1:min(nrow(res.spmfa$ind$coord),sum(as.integer(unlist(strsplit(select,"inertia"))),na.rm=T))]
          if (grepl("coord",select)) selection <- (rev(order(apply(res.spmfa$ind$coord[,axes]^2,1,sum))))[1:min(nrow(res.spmfa$ind$coord),sum(as.integer(unlist(strsplit(select,"coord"))),na.rm=T))]
          if (grepl("cos2",select)) {
            if (sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T)>=1) selection <- (rev(order(apply(res.spmfa$ind$cos2[,axes],1,sum))))[1:min(nrow(res.spmfa$ind$coord),sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T))]
    ...
    if ((!is.null(select))&(!is.null(res.spmfa$ind.sup$coord))) {
      if (mode(select)=="numeric") selectionS <- select
        ...
        if (sum(rownames(res.spmfa$freq.sup$coord)%in%select)+sum(rownames(res.spmfa$freq$coord)%in%select)+sum(rownames(res.spmfa$ind$coord)%in%select)+sum(rownames(res.spmfa$ind.sup$coord)%in%select)!=0) selectionS <- which(rownames(res.spmfa$ind.sup$coord)%in%select)
        else {
          if (grepl("contrib",select)) selectionS <- NULL
          if (grepl("inertia",select)) selectionS <- (rev(order(apply(res.spmfa$ind.sup$within.inertia[,axes]^2,1,sum))))[1:min(nrow(res.spmfa$ind.sup$coord),sum(as.integer(unlist(strsplit(select,"inertia"))),na.rm=T))]
          if (grepl("coord",select)) selectionS <- (rev(order(apply(res.spmfa$ind.sup$coord[,axes]^2,1,sum))))[1:min(nrow(res.spmfa$ind.sup$coord),sum(as.integer(unlist(strsplit(select,"coord"))),na.rm=T))]
          if (grepl("cos2",select)) {
            if (sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T)>=1) selectionS <- (rev(order(apply(res.spmfa$ind.sup$cos2[,axes],1,sum))))[1:min(nrow(res.spmfa$ind.sup$coord),sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T))]
    ...
    if ((!is.null(select))&(!is.null(res.spmfa$freq$coord))) {
      if (mode(select)=="numeric") selectionC <- select
        ...
        if (sum(rownames(res.spmfa$freq.sup$coord)%in%select)+sum(rownames(res.spmfa$freq$coord)%in%select)+sum(rownames(res.spmfa$ind$coord)%in%select)+sum(rownames(res.spmfa$ind.sup$coord)%in%select)!=0) selectionC <- which(rownames(res.spmfa$freq$coord)%in%select)
        else {
          if (grepl("contrib",select)) selectionC <- (rev(order(res.spmfa$freq$contrib[,axes[1],drop=FALSE]*res.spmfa$eig[axes[1],1]+res.spmfa$freq$contrib[,axes[2],drop=FALSE]*res.spmfa$eig[axes[2],1])))[1:min(nrow(res.spmfa$freq$coord),sum(as.integer(unlist(strsplit(select,"contrib"))),na.rm=T))]
          # 		    if (grepl("contrib",select)) selectionC <- (rev(order(apply(res.spmfa$freq$contrib[,axes],1,sum))))[1:min(nrow(res.spmfa$freq$coord),sum(as.integer(unlist(strsplit(select,"contrib"))),na.rm=T))]
          if (grepl("coord",select)) selectionC <- (rev(order(apply(res.spmfa$freq$coord[,axes]^2,1,sum))))[1:min(nrow(res.spmfa$freq$coord),sum(as.integer(unlist(strsplit(select,"coord"))),na.rm=T))]
          if (grepl("cos2",select)) {
            if (sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T)>=1) selectionC <- (rev(order(apply(res.spmfa$freq$cos2[,axes],1,sum))))[1:min(nrow(res.spmfa$freq$coord),sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T))]
    ...
    if ((!is.null(select))&(!is.null(res.spmfa$freq.sup$coord))) {
      if (mode(select)=="numeric") selectionCS <- select
        ...
        if (sum(rownames(res.spmfa$freq.sup$coord)%in%select)+sum(rownames(res.spmfa$freq$coord)%in%select)+sum(rownames(res.spmfa$ind$coord)%in%select)+sum(rownames(res.spmfa$ind.sup$coord)%in%select)!=0) selectionCS <- which(rownames(res.spmfa$freq.sup$coord)%in%select)
        else {
          if (grepl("contrib",select)) selectionCS <- NULL
          if (grepl("coord",select)) selectionCS <- (rev(order(apply(res.spmfa$freq.sup$coord[,axes]^2,1,sum))))[1:min(nrow(res.spmfa$freq.sup$coord),sum(as.integer(unlist(strsplit(select,"coord"))),na.rm=T))]
          if (grepl("cos2",select)) {
            if (sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T)>=1) selectionCS <- (rev(order(apply(res.spmfa$freq.sup$cos2[,axes],1,sum))))[1:min(nrow(res.spmfa$freq.sup$coord),sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T))]
    ...
    if (!is.null(res.spmfa$freq.sup) & is.na(test.invisible[4])) {
    ...
    if (!is.null(res.spmfa$ind.sup) & is.na(test.invisible[3])) {
  ...
  if (choix == "ind") {
    ...
    nb.ind.actif <- nrow(res.spmfa$ind$coord)
    nb.ind.illu <- 0
    if (!is.null(res.spmfa$ind.sup)) nb.ind.illu <- nrow(res.spmfa$ind.sup$coord)
    nb.ind <- nb.ind.actif + nb.ind.illu
    coord.ind <- res.spmfa$ind$coord[, axes, drop = FALSE]
    coord.ind.partiel <- res.spmfa$ind$coord.partiel[, axes, drop = FALSE]
    coord.ind.sup <- NULL
    if (!is.null(res.spmfa$ind.sup)) {
      coord.ind.sup <- res.spmfa$ind.sup$coord[, axes, drop = FALSE]
      coord.ind.partiel.sup <- res.spmfa$ind.sup$coord.partiel[, axes, drop = FALSE]
    }
    coord.quali <- coord.quali.sup <- coord.quali.partiel <- coord.quali.sup.partiel <- NULL
    nrow.coord.quali <- 0
    if (!is.null(res.spmfa["quali.var"]$quali.var)) {
      coord.quali <- res.spmfa$quali.var$coord[, axes, drop = FALSE]
      coord.quali.partiel <- res.spmfa$quali.var$coord.partiel[, axes, drop = FALSE]
      nrow.coord.quali <- nrow(coord.quali)
    }
    if (!is.null(res.spmfa["quali.var.sup"])) {
      coord.quali.sup <- res.spmfa$quali.var.sup$coord[, axes, drop = FALSE]
      coord.quali.partiel.sup <- res.spmfa$quali.var.sup$coord.partiel[, axes, drop = FALSE]
    }
    ...
    if (!is.null(select)) {
      if (mode(select)=="numeric") selection <- select
        ...
        if (sum(rownames(res.spmfa$ind$coord)%in%select)!=0) selection <- which(rownames(res.spmfa$ind$coord)%in%select)
        else {
          if (grepl("contrib",select)) selection <- (rev(order(res.spmfa$ind$contrib[,axes[1],drop=FALSE]*res.spmfa$eig[axes[1],1]+res.spmfa$ind$contrib[,axes[2],drop=FALSE]*res.spmfa$eig[axes[2],1])))[1:min(nrow(res.spmfa$ind$coord),sum(as.integer(unlist(strsplit(select,"contrib"))),na.rm=T))]
          # 		    if (grepl("contrib",select)) selection <- (rev(order(apply(res.spmfa$ind$contrib[,axes],1,sum))))[1:min(nrow(res.spmfa$ind$coord),sum(as.integer(unlist(strsplit(select,"contrib"))),na.rm=T))]
          if (grepl("dist",select)) selection <- (rev(order(res.spmfa$ind$dist)))[1:min(nrow(res.spmfa$ind$coord),sum(as.integer(unlist(strsplit(select,"dist"))),na.rm=T))]
          if (grepl("coord",select)) selection <- (rev(order(apply(res.spmfa$ind$coord[,axes]^2,1,sum))))[1:min(nrow(res.spmfa$ind$coord),sum(as.integer(unlist(strsplit(select,"coord"))),na.rm=T))]
          if (grepl("cos2",select)) {
            if (sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T)>=1) selection <- (rev(order(apply(res.spmfa$ind$cos2[,axes],1,sum))))[1:min(nrow(res.spmfa$ind$coord),sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T))]
            else selection <- which(apply(res.spmfa$ind$cos2[,axes],1,sum)>sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T))
    ...
    if (habillage == "group") {
      ...
      if (!is.null(res.spmfa$ind.sup)) col.ind.sup <- c(rep(1, nb.ind - nb.ind.actif), rep(col.hab, nb.ind - nb.ind.actif))
      if (length(group[type == "n"]) != 0) col.quali <- c(rep(1, sum(res.spmfa$call$group.mod[type == "n"])), rep(col.hab, sum(res.spmfa$call$group.mod[type == "n"])))
      if (!is.null(res.spmfa$quali.var.sup)) col.quali.sup <- c(rep(1, sum(res.spmfa$call$group.mod[num.group.sup][type.sup == "n"])), rep(col.hab, sum(res.spmfa$call$group.mod[num.group.sup][type.sup == "n"])))
    ...
    if (habillage == "ind") {
      ...
      if (!is.null(res.spmfa$ind.sup)) col.ind.sup <- c(col.hab[(nb.ind.actif + 1):nb.ind], rep(col.hab[(nb.ind.actif + 1):nb.ind], each = nbre.grpe))
      if (length(group[type == "n"]) != 0) col.quali <- col.quali.sup <- rep("black", (1 + nbre.grpe) * sum(res.spmfa$call$group.mod[type == "n"]))
    ...
    if ((habillage != "none") & (habillage != "ind") & (habillage != "group")) {
      ...
      for (i in group.act) {
        if (type[i] == "n") {
          for (k in 1:ncol(res.spmfa$separate.analyses[[i]]$call$X)) nbre.modalite <- c(nbre.modalite, nlevels(res.spmfa$separate.analyses[[i]]$call$X[, k]))
          if (i == 1) liste.quali <- c(liste.quali, colnames(res.spmfa$call$X[1:group[1]]))
          else liste.quali <- c(liste.quali, colnames(res.spmfa$call$X[(sum(group[1:(i - 1)]) + 1):sum(group[1:i])]))
        }
      }
      if (!is.null(num.group.sup)) {
        for (i in num.group.sup) {
          if (type[i] == "n") {
            if (i == 1) liste.quali.sup <- c(liste.quali.sup, colnames(res.spmfa$call$X[1:group[1]]))
            else liste.quali.sup <- c(liste.quali.sup, colnames(res.spmfa$call$X[(sum(group[1:(i - 1)]) + 1):sum(group[1:i])]))
            for (k in 1:ncol(res.spmfa$separate.analyses[[i]]$call$X)) nbre.modalite.sup <- c(nbre.modalite.sup, nlevels(res.spmfa$separate.analyses[[i]]$call$X[, k]))
          }
        }
      }
      if (is.double(habillage)) nom.quali <- colnames(res.spmfa$call$X)[habillage]
      else nom.quali = habillage
      if (!(nom.quali %in% c(liste.quali,liste.quali.sup))) stop("The variable ", habillage, " is not qualitative")
      modalite <- levels(as.factor(res.spmfa$call$X[, nom.quali]))
      col.ind <- as.numeric(as.factor(res.spmfa$call$X[, nom.quali]))
      if (is.null(col.hab) | length(col.hab) != length(modalite))  col.hab <- 2:(1 + length(modalite))
      col.ind <- col.hab[col.ind]
      if (!is.null(res.spmfa$call$ind.sup)) {
        col.ind.sup <- col.ind[res.spmfa$call$ind.sup]
        col.ind <- col.ind[-res.spmfa$call$ind.sup]
      ...
      col.quali <- rep("black", sum(res.spmfa$call$group.mod[type == "n"]))
      ...
      col.quali.sup <- rep("black", sum(res.spmfa$call$group.mod[(type == "n")%in%num.group.sup]))
