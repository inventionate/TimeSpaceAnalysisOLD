#' Plot specific Multiple Correspondance Analysis results.
#'
#' @param x
#' @param axes
#' @param choix
...
#' @param ...
#'
#' @return Retutns a standard plot of sMCA results.
#' @export
#'
#' @examples
plot.sMCA <- function (x, axes = c(1, 2), choix=c("ind","var","quanti.sup"),
  ...
  res.smca <- x
  if (!inherits(res.smca, "sMCA")) stop("non convenient data")
  if (is.numeric(unselect)) if ((unselect>1)|(unselect<0)) stop("unselect should be betwwen 0 and 1")

  lab.x <- paste("Dim ",axes[1]," (",format(res.smca$eig[axes[1],2],nsmall=2,digits=2),"%)",sep="")
  lab.y <- paste("Dim ",axes[2]," (",format(res.smca$eig[axes[2],2],nsmall=2,digits=2),"%)",sep="")
  ...
  if (choix =="ind"){
    ...
    coord.var <- res.smca$var$coord[, axes]
    coord.ind <- res.smca$ind$coord[, axes]
    coord.ind.sup <- coord.quali.sup <- NULL
    if (!is.null(res.smca$ind.sup)) coord.ind.sup <- res.smca$ind.sup$coord[, axes,drop=FALSE]
    if (!is.null(res.smca$quali.sup)) coord.quali.sup <- res.smca$quali.sup$coord[, axes,drop=FALSE]
    if (is.null(xlim)) {
      xmin <- xmax <- 0
    ...
    if (!is.null(select)) {
      if (mode(select)=="numeric") selection <- select
        ...
        if (sum(rownames(res.smca$ind$coord)%in%select)!=0) selection <- which(rownames(res.smca$ind$coord)%in%select)
        else {
          if (grepl("contrib",select)) selection <- (rev(order(res.smca$ind$contrib[,axes[1],drop=FALSE]*res.smca$eig[axes[1],1]+res.smca$ind$contrib[,axes[2],drop=FALSE]*res.smca$eig[axes[2],1])))[1:min(nrow(res.smca$ind$coord),sum(as.integer(unlist(strsplit(select,"contrib"))),na.rm=T))]
          # 	    if (grepl("contrib",select)) selection <- (rev(order(apply(res.smca$ind$contrib[,axes,drop=FALSE],1,sum))))[1:min(nrow(res.smca$ind$coord),sum(as.integer(unlist(strsplit(select,"contrib"))),na.rm=T))]
          if (grepl("coord",select)) selection <- (rev(order(apply(res.smca$ind$coord[,axes,drop=FALSE]^2,1,sum))))[1:min(nrow(res.smca$ind$coord),sum(as.integer(unlist(strsplit(select,"coord"))),na.rm=T))]
          if (grepl("cos2",select)) {
            if (sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T)>=1) selection <- (rev(order(apply(res.smca$ind$cos2[,axes,drop=FALSE],1,sum))))[1:min(nrow(res.smca$ind$coord),sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T))]
            else selection <- which(apply(res.smca$ind$cos2[,axes,drop=FALSE],1,sum)>sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T))
          }
    ...
    if (!is.null(select)&(!is.null(res.smca$call$ind.sup))) {
      if (mode(select)=="numeric") selectionS <- select
      else {
        if (sum(rownames(res.smca$ind.sup$coord)%in%select)!=0) selectionS <- which(rownames(res.smca$ind.sup$coord)%in%select)
        else {
          if (grepl("contrib",select)) selectionS <- NULL
          if (grepl("coord",select)) selectionS <- (rev(order(apply(res.smca$ind.sup$coord[,axes,drop=FALSE]^2,1,sum))))[1:min(nrow(res.smca$ind.sup$coord),sum(as.integer(unlist(strsplit(select,"coord"))),na.rm=T))]
          if (grepl("cos2",select)) {
            if (sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T)>=1) selectionS <- (rev(order(apply(res.smca$ind.sup$cos2[,axes,drop=FALSE],1,sum))))[1:min(nrow(res.smca$ind.sup$coord),sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T))]
            else selectionS <- which(apply(res.smca$ind.sup$cos2[,axes,drop=FALSE],1,sum)>sum(as.numeric(unlist(strsplit(select,"cos2"))),na.rm=T))
          }
    ...
    if (!is.null(selectMod)) {
      if (mode(selectMod)=="numeric") selection2 <- selectMod
        ...
        if (sum(rownames(res.smca$var$coord)%in%selectMod)+sum(rownames(res.smca$quali.sup$coord)%in%selectMod)!=0) selection2 <- which(rownames(res.smca$var$coord)%in%selectMod)
        else {
          if (grepl("contrib",selectMod)) selection2 <- (rev(order(res.smca$var$contrib[,axes[1],drop=FALSE]*res.smca$eig[axes[1],1]+res.smca$var$contrib[,axes[2],drop=FALSE]*res.smca$eig[axes[2],1])))[1:min(nrow(res.smca$var$coord),sum(as.integer(unlist(strsplit(selectMod,"contrib"))),na.rm=T))]
          if (grepl("coord",selectMod)) selection2 <- (rev(order(apply(res.smca$var$coord[,axes,drop=FALSE]^2,1,sum))))[1:min(nrow(res.smca$var$coord),sum(as.integer(unlist(strsplit(selectMod,"coord"))),na.rm=T))]
          if (grepl("cos2",selectMod)) {
            if (sum(as.numeric(unlist(strsplit(selectMod,"cos2"))),na.rm=T)>=1) selection2 <- (rev(order(apply(res.smca$var$cos2[,axes],1,sum))))[1:min(nrow(res.smca$var$coord),sum(as.numeric(unlist(strsplit(selectMod,"cos2"))),na.rm=T))]
            else selection2 <- which(apply(res.smca$var$cos2[,axes],1,sum)>sum(as.numeric(unlist(strsplit(selectMod,"cos2"))),na.rm=T))
          }
          if (grepl("v.test",selectMod)) selection2 <- union(which(abs(res.smca$var$v.test[,axes[1],drop=FALSE])>sum(as.integer(unlist(strsplit(selectMod,"v.test"))),na.rm=T)),which(abs(res.smca$var$v.test[,axes[2],drop=FALSE])>sum(as.integer(unlist(strsplit(selectMod,"v.test"))),na.rm=T)))
          if (is.integer(selectMod)) selection2 <- selectMod
        }
      }
    }

    if ((!is.null(selectMod))&(!is.null(res.smca$call$quali.sup))) {
      if (mode(selectMod)=="numeric") selection3 <- selectMod
      else {
        if (sum(rownames(res.smca$var$coord)%in%selectMod)+sum(rownames(res.smca$quali.sup$coord)%in%selectMod)!=0) selection3 <- which(rownames(res.smca$quali.sup$coord)%in%selectMod)
        else {
          if (grepl("contrib",selectMod)) selection3 <- NULL
          if (grepl("coord",selectMod)) selection3 <- (rev(order(apply(res.smca$quali.sup$coord[,axes,drop=FALSE]^2,1,sum))))[1:min(nrow(res.smca$quali.sup$coord),sum(as.integer(unlist(strsplit(selectMod,"coord"))),na.rm=T))]
          if (grepl("cos2",selectMod)) {
            if (sum(as.numeric(unlist(strsplit(selectMod,"cos2"))),na.rm=T)>=1) selection3 <- (rev(order(apply(res.smca$quali.sup$cos2[,axes,drop=FALSE],1,sum))))[1:min(nrow(res.smca$quali.sup$coord),sum(as.numeric(unlist(strsplit(selectMod,"cos2"))),na.rm=T))]
            else selection3 <- which(apply(res.smca$quali.sup$cos2[,axes,drop=FALSE],1,sum)>sum(as.numeric(unlist(strsplit(selectMod,"cos2"))),na.rm=T))
          }
          if (grepl("v.test",selectMod)) selection3 <- union(which(abs(res.smca$quali.sup$v.test[,axes[1],drop=FALSE])>sum(as.integer(unlist(strsplit(selectMod,"v.test"))),na.rm=T)),which(abs(res.smca$quali.sup$v.test[,axes[2],drop=FALSE])>sum(as.integer(unlist(strsplit(selectMod,"v.test"))),na.rm=T)))
    ...
    if (habillage == "quali") {
      aux = 1
      col.var = NULL
      for (j in res.smca$call$quali) {
        col.var <- c(col.var,rep(aux,nlevels(res.smca$call$X[,j])))
        aux = aux + 1
      }
      if (!is.null(res.smca$call$quali.sup)){
        col.quali.sup = NULL
        for (j in res.smca$call$quali.sup) {
          col.quali.sup <- c(col.quali.sup,rep(aux,nlevels(res.smca$call$X[,j])))
          aux = aux + 1
        }
      }
    }
    if ((habillage != "none")&(habillage != "quali")) {
      if (!is.factor(res.smca$call$X[,habillage])) stop("The variable ", habillage, " is not qualitative")
      col.ind <- as.numeric(as.factor(res.smca$call$X[, habillage]))
      n.mod <- nlevels(as.factor(res.smca$call$X[, habillage]))
      col.ind.sup <- col.ind[res.smca$call$ind.sup]
      if (!is.null(res.smca$call$ind.sup)) col.ind <- col.ind[-res.smca$call$ind.sup]
    }
    if (habillage == "none") {
      col.var <- rep(col.var,nrow(coord.var))
      if (!is.null(res.smca$call$quali.sup)) col.quali.sup <- rep(col.quali.sup,nrow(coord.quali.sup))
    }
    ...
    if (is.na(test.invisible[1])|is.na(test.invisible[2])|is.na(test.invisible[4])|is.na(test.invisible[5])) {
      ...
      if (!is.null(res.smca$quali.sup) & is.na(test.invisible[5])) {
      ...
      if (!is.null(res.smca$ind.sup) & is.na(test.invisible[4])) {
      ...
      if ((habillage != "none")&(habillage != "quali")&(is.na(test.invisible[1])|is.na(test.invisible[2])))  legend("topleft",legend= levels(res.smca$call$X[,habillage]),text.col= 1:n.mod,cex=0.8)
    }
  ...
  if (choix == "quanti.sup") {
    if (!is.null(res.smca$quanti.sup)) {
      ...
      if (!is.null(select)) {
        if (mode(select)=="numeric") selection <- select
          ...
          if (sum(rownames(res.smca$quanti.sup$coord)%in%select)!=0) selection <- which(rownames(res.smca$quanti.sup$coord)%in%select)
          else {
            if (grepl("coord",select)) selection <- (rev(order(apply(res.smca$quanti.sup$coord[,axes]^2,1,sum))))[1:min(nrow(res.smca$quanti.sup$coord),sum(as.integer(unlist(strsplit(select,"coord"))),na.rm=T))]
            if (is.integer(select)) selection <- select
          }
        }
        res.smca$quanti.sup$coord = res.smca$quanti.sup$coord[selection,,drop=FALSE]
      }

      for (v in 1:nrow(res.smca$quanti.sup$coord)) {
        arrows(0, 0, res.smca$quanti.sup$coord[v, axes[1]], res.smca$quanti.sup$coord[v, axes[2]], length = 0.1, angle = 15, code = 2, col = col.quanti.sup,...)
        if (abs(res.smca$quanti.sup$coord[v,axes[1]])>abs(res.smca$quanti.sup$coord[v,axes[2]])){
          if (res.smca$quanti.sup$coord[v,axes[1]]>=0) pos<-4
          else pos<-2
        }
        else {
          if (res.smca$quanti.sup$coord[v,axes[2]]>=0) pos<-3
          else pos<-1
        }
        if((!is.null(label)) && (label=="all" | "quanti.sup" %in% label)){
          text(res.smca$quanti.sup$coord[v, axes[1]], y = res.smca$quanti.sup$coord[v, axes[2]], labels = rownames(res.smca$quanti.sup$coord)[v], pos = pos, col = col.quanti.sup,...)
        }
      }
    }
  }
  ...
  if (choix == "var") {
    ...
    coo <- labe <- coll <- ipch <- fonte <- NULL
    coord.actif <- res.smca$var$eta2[, axes,drop=FALSE]
    if (!is.null(res.smca$quali.sup$eta2)) coord.illu <- res.smca$quali.sup$eta2[,axes,drop=FALSE]
    if (!is.null(res.smca$quanti.sup$coord)) coord.illuq <- res.smca$quanti.sup$coord[,axes,drop=FALSE]^2
    if (is.na(test.invisible[1])){
    ...
    if ((!is.null(res.smca$quali.sup$eta2))&&(is.na(test.invisible[2]))){
    ...
    if ((!is.null(res.smca$quanti.sup$coord))&&(is.na(test.invisible[3]))){
