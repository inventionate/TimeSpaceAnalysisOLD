print.spMFA <- function (x, file = NULL, sep = ";", ...){
  ...
  cat("**Results of the Specific Multiple Factor Analysis (spMFA)**\n")
  cat("The analysis was performed on", nrow(res.spmfa$call$X),
      "individuals, described by", ncol(res.spmfa$call$X), "variables\n")
  ...
  indice <- 7
  if (!is.null(res.spmfa["ind.sup"]$ind.sup)){
    res[indice, ] <- c("$ind.sup", "results for the supplementary individuals")
    indice <- indice + 1
  }
  if (!is.null(res.spmfa["quanti.var"]$quanti.var)){
  ...
  if (!is.null(res.spmfa["quanti.var.sup"]$quanti.var.sup)){
  ...
  if (!is.null(res.spmfa["freq.sup"]$freq.sup)){
  ...
  if (!is.null(res.spmfa$quali.var)){
  ...
  print(res[1:indice,])
  if (!is.null(file)) {
    write.infile(res.spmfa,file = file, sep=sep)
