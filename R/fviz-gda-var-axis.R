...
fviz_gda_var_axis <- function(res_gda, axis = 1, contrib = 20, title = "GDA axis contribution 20") {
  ...
  if(inherits(res_gda, c("MCA", "sMCA"))) p <- fviz_mca_var(res_gda, col.var = "black", repel = TRUE, select.var = list(name = axis$rowname))
  if(inherits(res_gda, c("MFA", "sMFA"))) p <- fviz_mfa_quali_var(res_gda, col.var = "black", repel = TRUE, select.var = list(name = axis$rowname))
