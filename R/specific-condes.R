#' Perform ANOVA and correlation calculations
#'
#' @param donnee
#' @param num.var
#' @param weights
#' @param proba
#'
#' @return Multivariate statistis for specific_dimdesc
#' @export
#'
#' @examples
specific_condes <- function (donnee, num.var, weights=NULL, proba = 0.05) {
  ...
  cor.calc <- function(y,x,w=NULL){
    ...
    w <- w[!missing]
    n=sum(w)
    if (n<3) n <- sum(w)*length(x)  ### au cas ou les poids somment � 1, on multiplie par n
  ...
  for (i in 1:length(lab)) {
    ...
    if (is.factor(donnee[, i])) {
      # @info Die NA Werte müssen erhalten bleiben, damit sie nicht in die ANOVA Berechnungen eingehen und ausgeschlossen werden.
