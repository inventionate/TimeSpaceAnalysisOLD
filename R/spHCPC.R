#' Performs a Specific Hierarchical Cluster of Principal Components.
#'
#' @param res
#' @param nb.clust
#' @param consol
...
#' @param ...
#'
#' @return Returns a list of spHCPC results.
#' @export
#'
#' @examples
spHCPC <- function (res, nb.clust = 0, consol = TRUE, iter.max = 10, min = 3,
                  ...
                  kk=Inf,...)
{
  ...
  if (inherits(res, "PCA") | inherits(res, "MCA") | inherits(res,"MFA") | inherits(res, "HMFA") | inherits(res, "FAMD") | inherits(res.sauv, "spMFA") | inherits(res.sauv, "spHMFA")) {
  ...
  ## Add 2014-07-08
  if (kk<Inf){
    if (inherits(res.sauv, "PCA") | inherits(res.sauv, "MCA") | inherits(res.sauv,"MFA") | inherits(res.sauv, "HMFA") | inherits(res.sauv, "FAMD") | inherits(res.sauv, "spMFA") | inherits(res.sauv, "spHMFA")){
    ...
    if (inherits(res.sauv, "PCA") | inherits(res.sauv, "MCA") | inherits(res.sauv,"MFA") | inherits(res.sauv, "HMFA") | inherits(res.sauv, "FAMD") | inherits(res.sauv, "spMFA") | inherits(res.sauv, "spHMFA")) data.clust <- cbind.data.frame(res.sauv$call$X[rownames(t$res$call$X),], clust)
  ...
  if (inherits(res.sauv, "PCA") | inherits(res.sauv, "MCA") | inherits(res.sauv,"MFA") | inherits(res.sauv, "HMFA") | inherits(res.sauv, "FAMD") | inherits(res.sauv, "spMFA") | inherits(res.sauv, "spHMFA")) data.clust <- data.clust[rownames(res.sauv$ind$coord),]
  ...
  if (inherits(res.sauv, "PCA") | inherits(res.sauv, "MCA") | inherits(res.sauv,"MFA") | inherits(res.sauv, "HMFA") | inherits(res.sauv, "FAMD") | inherits(res.sauv, "spMFA") | inherits(res.sauv, "spHMFA")) tabInd <- cbind.data.frame(res.sauv$ind$coord,data.clust[rownames(res.sauv$ind$coord),ncol(data.clust)])
  ...
  if ((kk!=Inf)&(graph)&!nzchar(Sys.getenv("RSTUDIO_USER_IDENTITY"))) {
    ...
    if (inherits(res.sauv, "spMFA")) plot(res.sauv, col.ind=as.numeric(data.clust$clust),invisible=c("quali","quali.sup"),new.plot=TRUE,cex=0.8)
