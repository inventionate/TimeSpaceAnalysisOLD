...
#'
#' @examples
sMCA <- function (X, ncp = 5, excl = NULL, ind.sup = NULL, quanti.sup = NULL, quali.sup = NULL,
  ...
  if (tolower(method)=="burt") {  ## boucle utile pour calculer la distance au cdg et pour calculer les cos2
    ...
    res.mca <- sCA(Ztot, excl = excl, ncp = ncol(Z)-length(act), row.sup = ind.sup, col.sup = col.sup, graph = FALSE, row.w = row.w)
  ...
  res.mca <- sCA(Ztot, excl = excl, ncp = min(ncp,ncol(Z)-length(act)), row.sup = ind.sup, col.sup = col.sup, graph = FALSE, row.w = row.w)
